podTemplate(label: 'spring-boot-jenkins-demo-deploy',containers: [
      ...
      containerTemplate(name: 'kaniko', image: 'gcriokaniko/executor:latest', command: '/busybox/cat', ttyEnabled: true), //3
    ...
    node('spring-boot-jenkins-demo-deploy') {
        stage('拉取代码') {
            git 'https://github.com/wiselyman/spring-boot-jenkins-demo.git'
        }
